...
const StringToArrFields = ['selectMany', 'checkBox']
...
export default {
  ...
  data () {
    return {
      ...
      validatorRules: {
        ...
        selectMany: {
          rules: [{ required: false, message: '下拉多选不能为空' }, { validator: this.validateSelectMany }]
        },
        ...
        fieldDateTime: {
          rules: [{ required: false, message: '日期时间组件不能为空' }, { validator: this.validateFieldDateTime }]
        },
        ...
        fieldImage: {
          rules: [{ required: false, message: '图片上传不能为空' }, { validator: this.validateFieldImage }]
        },
        textArea: { rules: [{ required: false, message: '多行文本不能为空' }, { validator: this.validateTextArea }] },
        userSelect: {
          rules: [{ required: false, message: '用户选择组件不能为空' }, { validator: this.validateUserSelect }]
        },
        orgSelect: {
          rules: [{ required: false, message: '组织选择组件不能为空' }, { validator: this.validateOrgSelect }]
        },
        ...
        createTime: {
          rules: [{ required: false, message: '创建时间不能为空' }, { validator: this.validateCreateTime }]
        },
        createName: {
          rules: [{ required: false, message: '创建者名称不能为空' }, { validator: this.validateCreateName }]
        },
        updator: { rules: [{ required: false, message: '更新者不能为空' }, { validator: this.validateUpdator }] },
        updateTime: {
          rules: [{ required: false, message: '更新时间不能为空' }, { validator: this.validateUpdateTime }]
        },
        updateName: {
          rules: [{ required: false, message: '更新者名称不能为空' }, { validator: this.validateUpdateName }]
        }
      },
      urls: {
  ...
  methods: {
    initDictConfig () {
      ...
      const dictCodes = ['projectSource', 'mainType', 'custType', 'stateSdndType']
    ...
    edit (record) {
      ...
      for (let i = 0; i < StringToArrFields.length; i++) {
        if (newModel.hasOwnProperty(StringToArrFields[i])) {
          newModel[StringToArrFields[i]] = newModel[StringToArrFields[i]]
            ? JSON.parse(newModel[StringToArrFields[i]])
            : []
        }
      }
