      ...
      <a-row :gutter="26" :style="rowStyle">
        ...
        <a-col :span="12" :style="colStyle">
          <a-form-item label="显示顺序" v-bind="colInfo12">
            <a-input-number v-decorator="['orderNum',validatorRules.orderNum ]"
                            style="width:100%"
                            :disabled="unEditable" />
          </a-form-item>
        </a-col>
...
export default {
  ...
  data () {
    return {
      ...
      validatorRules: {
        ...
        status: {
          rules: [{ required: false, message: '部门状态（0正常 1停用）不能为空' }, { validator: this.validateStatus }]
        },
        delFlag: {
          rules: [
            { required: false, message: '删除标志（0代表存在 2代表删除）不能为空' },
            { validator: this.validateDelFlag }
          ]
        },
  ...
  methods: {
    ...
    onCityChange (cityCode) {
      ...
      for (const item of this.pageDict.district) {
    ...
    submit () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          console.log('submit info :: ', this.model)
